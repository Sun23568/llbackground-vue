  ...
  <div class="app-container">
    <el-table :data="list" v-loading="listLoading" border fit
      ...
      <el-table-column align="center" label="菜单" width="420">
        <template slot-scope="scope">
          <el-tag v-for="menu in scope.row.menus" :key="menu.menuId" v-text="menu.menuName"
                  style="margin-right: 3px;" type="primary"></el-tag>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          <div>
            <div v-for="permType in scope.row.permTypes" style="text-align: left">
              <span style="width: 100px;display: inline-block;text-align: right ">{{ permType.type }}：</span>
              <el-tag v-for="perm in permType.perms" :key="perm.permId" v-text="perm.permName"
      ...
      <el-table-column align="center" label="管理">
        <template slot-scope="scope">
          ...
          <!--          </el-button>-->
          <el-button type="primary" icon="edit" @click="updateAccess(scope.$index)" v-permission="'access:update'">修改
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form class="small-space" :model="tmpUser" label-position="left" label-width="100px"
        ...
        <el-form-item label="菜单" required>
          <el-checkbox-group v-model="tmpUser.menuIds">
            <el-checkbox v-for="menu in allMenu" :label="menu.menuId" :key="menu.menuId">
              <span>{{ menu.menuName }}</span>
            </el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        <el-form-item label="权限" required>
          <div v-for=" (permType,_index) in allPermission">
            <span style="width: 100px;display: inline-block;">
              <el-button :type="isPermNone(permType.type)?'':(isPermAll(permType.type)?'success':'primary')" size="mini"
                         style="width:80px;"
                         @click="checkAll(permType.type)">{{ permType.type }}</el-button>
            </span>
            <div style="display: inline-block;margin-left:20px;">
              <el-checkbox-group v-model="tmpUser.permissions">
                <el-checkbox v-for="perm in permType.perms" :label="perm.permId" @change="checkRequired(perm,_index)"
                             :key="perm.permId">
  ...
  methods: {
    ...
    updateAccess($index) {
      this.tmpUser = {
        ...
        permissions: this.list[$index].permTypes.flatMap(permType => permType.perms.map(item => item.permId)),
      };
